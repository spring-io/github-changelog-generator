...
Alternatively, the summary can be taken from an issue comment made by a member of the issue's owning organization.
To be used as the summary, a comment must begin with a configured prefix.
The prefix is not included in the summary.
When no such comment is found on the issue, its title is used as a fallback.
